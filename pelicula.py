...
# Perfil de velocidad: definimos r desde 0 hasta aR para incluir el tubo
r_total = np.linspace(0, aR, 200)

# Función para calcular vz (es 0 dentro del tubo r < R)
def calcular_vz(r_val):
    condicion = r_val >= R
    # vz = (rho*g*R^2 / 4*mu) * [1 - (r/R)^2 + 2*a^2 * ln(r/R)]
    return np.where(condicion, 
                    (rho * g * R**2 / (4 * mu)) * (1 - (r_val/R)**2 + 2 * (a**2) * np.log(r_val/R)), 
                    0)
...
with col1:
    ...
    # Flechas de velocidad (Vectores de flujo)
    z_arrows = np.linspace(2, 8, 8)
    r_arrows = np.linspace(R, aR, 5)
    for z in z_arrows:
        for ra in r_arrows:
            v_loc = (rho * g * R**2 / (4 * mu)) * (1 - (ra/R)**2 + 2 * (a**2) * np.log(ra/R))
            v_len = (v_loc / vz_max) * 1.2 if vz_max > 0 else 0
            ax1.arrow(ra, z, 0, -v_len, head_width=0.003, head_length=0.1, fc='blue', ec='blue')
...
with col2:
    ...
    # Dibujar el Tubo Sólido en la gráfica (Velocidad = 0)
    ax2.axvspan(0, R, color='#444444', alpha=0.8, label='Tubo (v=0)')
    
    # Dibujar la curva del perfil de velocidad
    ...
    ax2.scatter([aR], [vz_max], color='green', zorder=5, label='$v_{max}$ (Aire)')
...
# --- 3. ECUACIONES DE GOBIERNO (ABAJO) ---
st.divider()
...
st.info(f"💡 Visualización: La zona gris representa el tubo sólido. Nota cómo la velocidad es exactamente 0 en r = {R:.3f}m y crece parabólicamente hacia la interfase aire.")
